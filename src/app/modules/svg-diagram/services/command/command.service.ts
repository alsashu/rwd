...
export class CommandService implements ICommandService {
  ...
  public execute(command: ICommand, options: any = null): boolean {
    let res = null;
    try {
      const theOptions = this.calcOptions(command);
      console.log("Exécution commande", command.getDescription(), command, theOptions);
      res = command.execute();
      if (res) {
  ...
  public executeCommands(cmds: ICommand[], description: string = null) {
    if (cmds && cmds.length) {
      this.execute(new CompositeCommand(this.svgDiagramService).init(cmds, description));
    }
  }
  ...
  public beginGroup(description: string = null) {
    ...
    this.groupCommand = new CompositeCommand(this.svgDiagramService).init([], description);
