...
export class IOVariableMappingsCompareTreeRule extends ANodeFactoryRule {
  private params: any = {
    ...
    if (
      ...
      if (this.rulesNodeFactory.isTypeExtensionOfTypes(object.type, this.params.tIOVariableMappingCategoryType)) {
        ...
        if (node) {
          ...
          if (parentClassNode) {
            const doublon = this.rulesNodeFactory.findDoublonNode(object, parentClassNode);
            context.tIOVariableMappingNode = doublon ? doublon : node;
