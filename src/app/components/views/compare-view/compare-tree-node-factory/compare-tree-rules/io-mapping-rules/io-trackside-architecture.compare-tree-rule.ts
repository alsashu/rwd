...
// 1.	TracksideArchitecture: displaying all the equipment rooms with their associated
// IOVariables segregated based on the field object in a tree structure and
// all the equipment room objects in a flat structure.

// export class IOTrackSideArchitectureTreeMenuRule extends ASimpleNodeFactoryRule {
//   protected params = {
//     categoryId: "IOTrackSideArchitecture",
//     nodesConfig: [{ id: "IOTrackSideArchitecture", label: "TrackSide Architecture", addChildrenCount: true }],
//     categoryTypes: [],
//   };
// }
...
export class IOTrackSideArchitectureCompareTreeRule extends ANodeFactoryRule {
  private params: any = {
    categoryId: "IOTrackSideArchitecture",
    ...
    if (
      ...
      if (this.rulesNodeFactory.isTypeExtensionOfTypes(object.type, [this.params.equipmentRoomsCategoryType])) {
        ...
        if (node) {
          ...
          compareRulesNodeFactory.addNodeToLastNodesByTypeMap(node, mapPrefix);
        ...
        this.params.equipmentRoomNodesConfig.forEach((equipmentRoomNodeConfig: any) => {
          if (this.rulesNodeFactory.isTypeExtensionOfTypes(object.type, [equipmentRoomNodeConfig.type])) {
            ...
            const equipmentItemParentNode = compareRulesNodeFactory.getLastNodeByType(
              equipmentRoomNodeConfig.parentType,
              mapPrefix
            );
            if (equipmentItemParentNode) {
              ...
              if (equipmentItemNode) {
                ...
                compareRulesNodeFactory.addNodeToLastNodesByTypeMap(equipmentItemNode, mapPrefix);
                this.rulesNodeFactory.addNodeToParent(
                  equipmentItemNode,
                  compareRulesNodeFactory.getMainSibblingNode(equipmentItemParentNode, mapPrefix)
                );
              }
            }
          }
