...
export class VerificationTreeItemService {
  ...
  public getVerificationToBeVerifiedValue(node: any): any {
    const result = this.modelVerificationService.getObjectIsToBeVerifiedValue(node ? node.object : null);
    //console.log(`🔍 getVerificationToBeVerifiedValue for ${node?.object?.property?.nameWithPrefix || 'object'}: ${result}`);
    return result;
  }
  ...
  public getVerificationStateShortDisplayValue(node: any): string {
    ...
    // Debug logging for verification state retrieval
    // if (node?.object?.property?.nameWithPrefix && node.object.property.nameWithPrefix.includes("rail:")) {
    //   console.log(`🔍 Verification State Retrieval Debug:`, {
    //     propertyName: node.object.property.nameWithPrefix,
    //     toBeVerified: tbv,
    //     verificationState: vst,
    //     verificationStateType: typeof vst
    //   });
    // }
    
    const result = this.getVerificationShortState(vst);
    return result;
  }
