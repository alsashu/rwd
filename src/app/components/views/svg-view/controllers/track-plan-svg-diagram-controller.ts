...
export class TrackPlanSvgDiagramController extends ASubTypeSvgDiagramController {
  ...
  protected initLayers() {
    ...
    this.svgDiagramService.layerService.layerOptions.layers =
      this.svgDiagramService.layerService.layerOptions.layers.concat([
        ...
        },
        {
          ...
          applyToSvgObjectCB: (svgObject: any) => {
            // Only show TVD sections when layer is visible
            return svgObject.bo && svgObject.bo.type && 
                   (svgObject.bo.type === "GenericADM:tVDSection" || 
                    svgObject.bo.type === "tVDSection");
          },
        },
        {
          layerType: "typeLayer",
      ...
      // Ensure layer options are applied after initialization
      setTimeout(() => {
