...
export class IomappingViewComponent extends AViewComponent implements OnInit, IViewComponent, OnDestroy {
  ...
  public bo: any;
  public project: any;
  public isConfiguration: boolean = false;
  ...
  private getAssemblyOfProject(project: any): any {
    if (!project) {
      console.log("ERROR: No project provided");
      return null;
    }
    
    if (!this.config.boId) {
      console.log("ERROR: No boId in config");
      return null;
    }
    
    let assembly = null;
    
    // Check if this is a configuration view
    this.isConfiguration = this.config.isConfiguration === true;
    
    if (this.isConfiguration) {
      // For configuration objects, find assemblies that reference this configuration
      const assemblies = this.modelService.getObjectsFromTypes(project, [ModelConstService.ASSEMBLY_TYPE]);
      assembly = assemblies.find((a: any) => 
        a.assemblyConfigurationRef === this.config.boId
      );
      
      if (!assembly) {
        // If no assembly found, get the configuration object itself
        assembly = this.modelService.getObjectById(project, this.config.boId);
      }
    } else {
      // Handle regular assembly objects
      assembly = this.modelService.getObjectByItemIdAndObjectClassName(project, {
