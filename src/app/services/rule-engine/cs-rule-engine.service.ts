...
/**
 * Interface of the CsRuleEngineService
 */
export interface ICsRuleEngineService {
  executeRules(rules: IRule[]);
}

/**
 * Right editor CSharp rule engine service
 */
export class CsRuleEngineService implements ICsRuleEngineService {
  ...
  private doRunRules(rules: IRule[]) {
    this.setLog(this.translateService.translateFromMap("Running cs rule engine..."));
    ...
    this.modelLoadSaveService.saveSelectedProject().subscribe((res: any) => {
      this.apiService.executeCsRuleEngine(params).subscribe((res: any) => {
        this.mvcService.emit({ type: MvcConst.MSG_CS_RULE_ENGINE_EXECUTION_END });
        // json log
        this.apiService.getCsRuleEngineLogs(params).then((loadedData: any) => {
  ...
  private setLog(log: string) {
    this.mvcService.emit({ type: MvcConst.MSG_CS_RULE_ENGINE_LOG_CHANGED, log });
  }
  ...
  private setLogs(logData: any) {
    try {
      this.mvcService.emit({
        type: MvcConst.MSG_CS_RULE_ENGINE_LOG_CHANGED,
        log: logData && logData.log ? logData.log : "",
        logData,
      });
    } catch (e) {
      this.mvcService.emit({ type: MvcConst.MSG_CS_RULE_ENGINE_LOG_CHANGED, log: e.toString() });
    }
  }
}
